...
def get_excel_sheet(path, sheet):
    '''
    获取sheet内容
    Params:
        path: String Excel路径
        sheet: String sheet名称j
    Return:
        sheet
    '''
    _excel = get_excel_file(path)
    _sheet = _excel.sheet_by_name(sheet)
    return _sheet
...
def templet_excel(pvals, pfname, psindex, ptitle_rnt=0, ptitle_cnt=0, is_by_row=True, val_style=None,
    ...
    :return:
    """
    _workbook = open_workbook(pfname, formatting_info=True)
    _wb = copy(_workbook)
    if is_new_sheet:
...
def get_standard_name_list(ppath, ssheet):
    '''
    获取标准化名称列表
    Params:
        ppath: String 标准化文件路径
        ssheet: String 标准化内容所在sheet
    Return:
        标准化名称列表
    '''
    _sheet = get_excel_sheet(ppath, ssheet)
    _rvals = _sheet.col_values(1)[1:]
    return _rvals


def get_file_add(pfile, ppath):
    '''
    需要替换（遮挡）的文件,返回有效的系统目录名称
    Params:
        pfile:
        ppath:
    Return:
